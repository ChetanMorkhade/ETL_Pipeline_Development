...
mysql_engine_tgt = create_engine("mysql+pymysql://root:root@localhost:3306/stag_retaildwh")
...
def load_fact_sales_table():
    ...
    query = text("""
        ...
        FROM stag_retaildwh.sales_with_details
    """)
    try:
...
def load_inventory_levels_by_store_table():
    # Use fully qualified names for cross-schema insert
    query = text("""
        INSERT INTO retaildwh.inventory_levels_by_store (store_id, total_inventory)
        SELECT store_id, total_inventory
        FROM stag_retaildwh.aggregated_inventory_level
    """)
    try:
        with mysql_engine_tgt.begin() as conn:  # begin() handles commit/rollback
            logger.info("inventory_levels_by_store table started loading...")
            logger.info(str(query))
            result = conn.execute(query)
            logger.info(f"Rows inserted: {result.rowcount}")
            logger.info("inventory_levels_by_store table completed loading...")
    except Exception as e:
        logger.error(f"Error encountered while loading into inventory_levels_by_store: {e}", exc_info=True)
...
def load_monthly_sales_summary_table():
    ...
    query = text("""
        ...
        FROM stag_retaildwh.monthly_sales_summary_source
    """)
    try:
        with mysql_engine_tgt.begin() as conn:  # begin() handles commit/rollback
...
if __name__ == "__main__":
    ...
    load_inventory_levels_by_store_table()
